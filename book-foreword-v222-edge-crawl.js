(() => {
  const V222_BUILD = 'BOOK FOREWORD v2.2.2 EDGE CRAWL';
  const V222_CACHE = 'book-foreword-v222-edge-crawl-phone';
  ...
  function markBuild() {
    ...
    if (marker) {
      marker.dataset.build = V222_BUILD;
      marker.textContent = V222_BUILD;
    }
    const renderFacts = facts();
    if (renderFacts) {
      renderFacts.dataset.build = V222_BUILD;
      renderFacts.dataset.cache = V222_CACHE;
      renderFacts.dataset.edgeCrawl = 'measured';
      renderFacts.dataset.glyphDrop = 'v222';
      renderFacts.dataset.sealRefusal = 'armed';
      renderFacts.dataset.voidDrop = 'smooth';
    }
  }
  ...
  function drawMeasuredPerimeter(svg, index) {
    ...
    if (svg.classList.contains('v222-edge-crawl') && prev === next) return;
    ...
    const id = `v222-edge-${index}-${width}-${height}`.replace(/[^a-zA-Z0-9_-]/g, '-');
    const digits = `${PI_DIGITS}${PI_DIGITS}${PI_DIGITS}`;
    const symbols = Array(260).fill('π').join(' ');

    svg.classList.remove('v221-full-perimeter');
    svg.classList.add('v222-edge-crawl');
    ...
    svg.innerHTML = `
      ...
      <text class="pi-symbol-thread"><textPath class="v222-crawl-forward" href="#${id}-symbols" startOffset="0%">${symbols}</textPath></text>
      <text class="pi-digit-thread"><textPath class="v222-crawl-reverse" href="#${id}-digits" startOffset="52%">${digits}</textPath></text>
    `;
  }
  ...
  function startCrawl() {
    ...
    const tick = now => {
      ...
      document.querySelectorAll('.v222-crawl-forward').forEach((path, index) => {
        path.setAttribute('startOffset', `${((elapsed * 0.0031 + index * 5) % 100).toFixed(2)}%`);
      });
      document.querySelectorAll('.v222-crawl-reverse').forEach((path, index) => {
        path.setAttribute('startOffset', `${(100 - ((elapsed * 0.0026 + 52 + index * 3) % 100)).toFixed(2)}%`);
      });
  ...
  function triggerLocalSealFeedback(event) {
    ...
    if (closing) closing.classList.add('v222-refused');
  ...
  function createDropStage() {
    ...
    stage.className = 'void-drop-stage v222-void-drop-stage';
    ...
    clone.classList.add('void-drop-page-clone', 'v222-drop-page-clone');
  ...
  function startVoidDrop() {
    ...
    setTimeout(advanceBehindBlack, 980);
    setTimeout(() => {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bootV222, { once: true });
  } else {
    bootV222();
  }
})();
