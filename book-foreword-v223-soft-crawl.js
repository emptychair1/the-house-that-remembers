(() => {
  const V223_BUILD = 'BOOK FOREWORD v2.2.3 SOFT CRAWL';
  const V223_CACHE = 'book-foreword-v223-soft-crawl-phone';
  ...
  function markBuild() {
    ...
    if (marker) {
      marker.dataset.build = V223_BUILD;
      marker.textContent = V223_BUILD;
    }
    const renderFacts = facts();
    if (renderFacts) {
      renderFacts.dataset.build = V223_BUILD;
      renderFacts.dataset.cache = V223_CACHE;
      renderFacts.dataset.edgeCrawl = 'soft';
      renderFacts.dataset.glyphDrop = 'v223';
      renderFacts.dataset.sealRefusal = 'armed';
      renderFacts.dataset.voidDrop = 'flat';
    }
  }
  ...
  function drawMeasuredPerimeter(svg, index) {
    ...
    if (svg.classList.contains('v223-edge-crawl') && prev === next) return;
    ...
    const id = `v223-edge-${index}-${width}-${height}`.replace(/[^a-zA-Z0-9_-]/g, '-');
    const digits = `${PI_DIGITS}${PI_DIGITS}${PI_DIGITS}`;
    const symbols = Array(260).fill('π').join(' ');

    svg.classList.remove('v221-full-perimeter', 'v222-edge-crawl');
    svg.classList.add('v223-edge-crawl');
    ...
    svg.innerHTML = `
      ...
      <text class="pi-symbol-thread"><textPath class="v223-crawl-forward" href="#${id}-symbols" startOffset="0%">${symbols}</textPath></text>
      <text class="pi-digit-thread"><textPath class="v223-crawl-reverse" href="#${id}-digits" startOffset="52%">${digits}</textPath></text>
    `;
  }
  ...
  function startCrawl() {
    ...
    const tick = now => {
      ...
      document.querySelectorAll('.v223-crawl-forward').forEach((path, index) => {
        path.setAttribute('startOffset', `${((elapsed * 0.0026 + index * 5) % 100).toFixed(2)}%`);
      });
      document.querySelectorAll('.v223-crawl-reverse').forEach((path, index) => {
  ...
  function triggerLocalSealFeedback(event) {
    ...
    if (closing) closing.classList.add('v223-refused');
  ...
  function createDropStage() {
    ...
    stage.className = 'void-drop-stage v223-void-drop-stage';
    ...
    clone.className = 'book-page foreword-closing-page visual-page composition-locked-page v223-drop-page-clone';
  ...
  function startVoidDrop() {
    ...
    setTimeout(advanceBehindBlack, 1250);
    setTimeout(() => {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bootV223, { once: true });
  } else {
    bootV223();
  }
})();